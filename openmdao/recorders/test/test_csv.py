...
class TestCsvRecorder(unittest.TestCase):
    def setUp(self):
        self.dir = mkdtemp()
        self.filename = os.path.join(self.dir, "csv_test.csv")
        self.recorder = CsvRecorder(self.filename)
        self.eps = 1e-5
    ...
    def assertIterationDataRecorded(self, expected, tolerance):
        ...
        with open(self.filename) as sout:
            csv_reader = csv.DictReader(sout)
            for row in csv_reader:
                for header_name in row:
                    if not header_name in saved_results:
            ...
            for coord, (t0, t1), params, unknowns, resids in expected:
                ...
                if unknowns is not None:
                    for unkown in unknowns:
                        self.assertTrue(unkown[0] in saved_results)
                        self.assertTrue(tuple(saved_results[unkown[0]]) == unkown, tuple(saved_results[unkown[0]]) + unkown)
